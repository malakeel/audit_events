...
static bool are_equal(HeapTupleHeader record1 , HeapTupleHeader record2) ;
...
static void audit_plugin_startup(LogicalDecodingContext *ctx, OutputPluginOptions *opt, bool is_init)
{
  ...
  for (idx = 0; idx < num_att; idx++)
	...
	field->new_val = (Datum) NULL;
      ...
      if(oldtuple != NULL && newtuple != NULL)
      	dirty = dirty && are_equal(oldtuple->t_data , newtuple->t_data) ;
...
bool are_equal(	HeapTupleHeader record1 , HeapTupleHeader record2)
{
  ...
  fcinfo.arg[0] = record1;
  fcinfo.arg[1] = record2;
